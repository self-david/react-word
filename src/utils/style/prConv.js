...
class PrConv {
  ...
  static convToPr(pr, conv) {
    for (const attrConv of this.attrConvs) {
      attrConv.to(pr, conv)
    }
    return conv
  }
...
const boolAttrs = [
  ...
  'i',
  'caps',
  'smallCaps', // caps-->all_caps  small_caps
  'strike',
  'outline',
] //, 'rtl'
...
class RPrConv extends PrConv {
  ...
  static convToPr(pr, conv) {
    for (const attr of boolAttrs) {
      const bool = conv[attr]
      if (bool === true || bool === false) {
        pr._set_bool_val(attr, bool)
      }
    }
    for (const attr of boolAttrs2) {
      const bool = conv[attr]
      if (bool === true || bool === false) {
        pr[attr] = bool
      }
    }
    if (conv.size) pr.sz_val = new shared.Pt(conv.size)
    if (conv.color) pr.get_or_add_color().val = conv.color
    if (conv.highlight) {
      pr.highlight_val = enums.WD_COLOR.from_xml(conv.highlight)
    }
    if (conv.underline) pr.u_val = conv.underline
    if (conv.name) {
  ...
  static prToConv(pr) {
    ...
    const { sz_val, color, highlight_val, u_val, rFonts_ascii } = pr
    if (sz_val) conv.size = sz_val.pt
    if (color) {
    ...
    if (u_val) conv.underline = u_val
    if (rFonts_ascii) conv.name = rFonts_ascii
...
class PPrConv extends PrConv {
  static attrConvs = [IndentSpacingConv, AlignmentConv, LineSpacingConv, ShdConv, PBordersConv]
