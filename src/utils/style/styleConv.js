...
class StyleConv {
  ...
  get baseId() {
    if (this.style.base_style) return this.style.base_style.style_id
  }

  get conv() {
    if (!this._conv) this._conv = this.constructor.fromStyle(this.style)
    return this._conv
  }
  ...
  get styleObj() {
    if (!this._styleObj) this._styleObj = this.constructor.toStyleObj(this.conv)
  ...
  static fromStyle(style) {
    const pr = this.prConv.getStylePr(style)
    if (pr) {
      const h = hash(serializeToString(pr.xmlElement))
      const c = cache.get(h)
      if (c) {
        return { ...c }
      }
      const conv = this.prConv.prToConv(pr)
      cache.set(h, conv)
      return conv
    } else {
      return {}
...
class FormatConv extends StyleConv {
  ...
  get conv() {
    if (!this._conv) {
      const base = this.styleMap.idToFormat(this.baseId)
      const conv = this.constructor.fromStyle(this.style)
      // let defaultFormat = this.styleMap.defaultFormat;
      if (base) this._conv = { ...base.conv, ...conv } // ...defaultFormat,
      else this._conv = conv // {...defaultFormat, ...conv};
    }
    return this._conv
  }

  get numberingObj() {
    const numbering = this.numberingMap.get(this.styleId)
    return numbering ? numbering.styleObj : null
  }

  static toStyle(paragraph, conv) {
    const pr = paragraph._element.get_or_add_pPr()
...
class FontConv extends StyleConv {
  ...
  get conv() {
    if (!this._conv) {
      const base = this.styleMap.idToFont(this.baseId)
      const conv = this.constructor.fromStyle(this.style)
      // let defaultFont = this.styleMap.defaultFont;
      if (base) this._conv = { ...base.conv, ...conv } // ...defaultFont
      else this._conv = conv // {...defaultFont, ...conv};
    }
    return this._conv
  }

  get linkId() {
    const elem = this.style._element
    const link = elem.find('w:link')
    if (link) {
  ...
  static runToText(run, parFontConv) {
    ...
    if (parFontConv) textConv = { ...parFontConv, ...textConv }
  ...
  static runFromText(run, textConv) {
    ...
    const pr = run._element.get_or_add_rPr()
