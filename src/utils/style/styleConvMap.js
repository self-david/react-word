...
class StyleConvMap {
  ...
  // }
  addStyle(style) {
    const conv = this.newConv(style)
    this.addConv(conv)
  }

  addConv(conv) {
    const { styleId, name } = conv
  ...
  getStyleObj(name) {
    const conv = this.nameTo.get(name)
    if (conv) return conv.styleObj
  }
...
class FormatConvMap extends StyleConvMap {
  ...
  mergeStyleNames(curStyleMap) {
    const names = curStyleMap ? curStyleMap.formatConvMap.styleNames : []
    for (const name of this.nameTo.keys()) {
...
class FontConvMap extends StyleConvMap {
  ...
  addConv(conv, formatConvMap) {
    const { styleId, name, linkId } = conv
    this.idTo.set(styleId, conv)
    this.nameTo.set(name, conv)
    this.idToName.set(styleId, name)
    if (linkId) {
      const formatName = formatConvMap.idToName.get(linkId)
...
class TableStyleConvMap extends StyleConvMap {
  ...
  getStyleObj(options) {
    const { style, look } = options
    const conv = this.nameTo.get(style)
    // console.log('table style name', style)
    if (conv) return conv.getCxClassWithLook(look)
  }
}
