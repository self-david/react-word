  ...
  editor.deleteBackward = unit => {
    ...
    if (selection && Range.isCollapsed(selection)) {
      const [cell] = Editor.nodes(editor, {
        match: n => !Editor.isEditor(n) && SlateElement.isElement(n) && n.type === 'cell',
      })
      if (cell) {
        const [, cellPath] = cell
  ...
  editor.deleteFragment = () => {
    ...
    if (selection) {
      ...
      if (cell0 && cell1) {
        ...
        if (Point.isAfter(selectionEnd, cellEnd) || Point.isBefore(selectionStart, cellStart)) {
          return
        }
      }
    }
...
const Table = ({ attributes, children, element }) => {
  if (element) {
    ...
    const cssClass = editor.getTableFormatStyle(element)
    const { styleObj } = element.styleConv
    return (
